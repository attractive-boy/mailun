...
class Result extends Component {
  constructor(props) {
    ...
    this.moduleData = decodeURIComponent(
      Taro.getCurrentInstance().router.params.results
    ); // 替换为实际参数名
    // 解析 JSON 字符串
    const parsedData = JSON.parse(this.moduleData);
    this.responseId = Taro.getCurrentInstance().router.params.responseId;
    // 转换为所需格式
    const convertedData = {};

    Object.keys(parsedData).forEach((wheel) => {
      convertedData[wheel] = Object.keys(parsedData[wheel]).map(
        (dimension) => ({
          dimension: dimension,
      ...
      <View className='page'>
        <View className='doc-body bg'>
          ...
          <AtButton
            ...
            onClick={() => {
              Taro.navigateTo({ url: "/pages/consult/index?responseId=" + this.responseId });
            }}
          >
            填写咨询单
          </AtButton>
