...
const request = async (url, options = {}) => {
  ...
  // 合并自定义请求头
  const headers = { ...defaultHeaders, ...options.header };
  ...
  try {
    ...
    if (res.statusCode === 200) {
      ...
      // 如果状态码是 401，表示 Token 过期或无效，跳转到登录页面
      Taro.removeStorageSync('token'); // 清除过期的 token
      Taro.showToast({
        title: '登录已过期，请重新登录',
        icon: 'none',
      });
      Taro.redirectTo({
        url: '/pages/login/index', // 重新定向到登录页面
      });
      throw new Error('Token 已过期');
    } else {
      Taro.showToast({
        title: `请求失败: ${res.data.message || '请稍后再试'}`,
        icon: 'none',
      });
...
export default {
  ...
  request,
  submitAnswers: async (questionnaireId, answers) => {
    try {
      // 构建请求数据
      const data = {
        questionnaireId,
        answers, // 用户的答案 [{ questionId: 1, selectedOptionId: 2 }, ...]
      };
